...
const AppointmentCard = ({
  ...
  const appPets = appointment.petIds
    .map((pId) => {
      if (adminCtx) {
  ...
  return (
    <Card
      elevation={2}
      style={{ marginVertical: 6, padding: 6, position: "relative" }}
    >
      <Card.Title
        title={
          <Title style={{ fontWeight: "bold", textTransform: "uppercase" }}>
        ...
        left={(props) =>
          appointment?.photoURL ? (
            ...
            <Avatar.Icon
              icon={
                appointment?.gender === "Female" ? "face-woman" : "face-man"
              }
              {...props}
            />
          )
      ...
      <Card.Content>
        <View style={{ marginBottom: 8 }}>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <Caption style={{ fontWeight: "bold", color: "#5C797C" }}>
              {moment(appointment?.day).format("ll")}
            </Caption>
          </View>
          <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            <Caption style={{ fontWeight: "bold", color: "#5C797C" }}>
              {appointment?.purpose.map((list, index) => {
                if (list.length > 1) {
                  return (index ? " & " : "") + list;
                } else {
                  return list;
